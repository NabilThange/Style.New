...
export const OutfitGenerator: React.FC<OutfitGeneratorProps> = ({
  ...
  useEffect(() => {
    const processQueue = async () => {
      ...
      // Double check API key before processing queue
      if (!apiKey) {
        setIsGenerating(false);
        setGenerationQueue([]); // Stop queue
        onOpenApiKeyModal();
        return;
      }
      ...
      try {
        ...
        const generatedImage = await generateOutfitImage(apiKey, person, upper, lower, currentOutfit.modelId || selectedModel, currentOutfit.imageSize || '1K');
  ...
  }, [isGenerating, generationQueue, apiKey]);
